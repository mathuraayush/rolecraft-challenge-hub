...
interface Profile {
  id: string; name: string | null; city: string | null; college: string | null;
  bio: string | null; github_url: string | null; linkedin_url: string | null;
  role: string | null; level: string | null;
}
interface Portfolio { user_id: string; is_public: boolean; headline: string | null; is_available_for_hire: boolean }
interface SubRow {
  id: string; status: string; ai_score: number | null; ai_feedback: string | null;
  approach_text: string | null; proposed_solution: string | null;
  submission_link: string | null; submission_type: string | null; created_at: string;
...
function PublicPortfolio() {
  ...
  const { user } = useAuth();
  const { recruiter, isRecruiter } = useRecruiter();
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: s } = await supabase
        ...
        .eq("user_id", id).in("status", ["submitted", "graded"]).order("created_at", { ascending: false });
  ...
  const fullAccess = isOwner || subscribed;
  ...
  const gradedCount = subs.filter((s) => s.status === "graded" && (s.ai_score ?? 0) > 0).length;
  const avgScore = gradedCount > 0
    ? Math.round(subs.filter((s) => (s.ai_score ?? 0) > 0).reduce((sum, s) => sum + (s.ai_score || 0), 0) / gradedCount)
    : 0;
        ...
        <div className="flex flex-col gap-6 sm:flex-row sm:items-start">
          ...
          <div className="flex-1">
            ...
            <div className="mt-3 flex flex-wrap gap-2 text-xs">
              ...
              {portfolio?.is_available_for_hire && <span className="rounded-full bg-success/15 px-2.5 py-1 font-medium text-success">● Open to opportunities</span>}
            </div>
          ...
          <div className="flex flex-col items-end gap-3">
            {gradedCount > 0 && (
              <div className="text-center">
                <div className={`font-display text-4xl font-semibold ${avgScore >= 80 ? "text-success" : avgScore >= 60 ? "text-primary" : "text-accent-foreground"}`}>{avgScore}</div>
                <div className="text-xs text-muted-foreground">Portfolio Score</div>
                <div className="mt-1 text-xs text-muted-foreground">{gradedCount} {gradedCount === 1 ? "project" : "projects"}</div>
              </div>
            )}
            {isOwner && !editing && (
      ...
      <section className="mt-10 relative">
        ...
        {subs.length === 0 ? (
          ...
          <div className="relative">
            <div className="mt-6 space-y-4" style={gated ? { filter: "blur(5px)", pointerEvents: "none", userSelect: "none" } : undefined}>
              {subs.map((s) => (
            ...
            {gated && (
              <div className="absolute inset-0 flex items-start justify-center pt-20">
                <div className="max-w-md rounded-2xl border border-border bg-card p-6 text-center shadow-xl">
                  {!user ? (
                    <>
                      <p className="text-base">Create a free recruiter account to browse candidate portfolios</p>
                      <Link to="/auth" className="mt-4 inline-flex rounded-xl bg-primary px-5 py-2.5 text-sm font-medium text-primary-foreground hover:opacity-90">
                        Sign up as Recruiter →
                      </Link>
                    </>
                  ) : isRecruiter ? (
                    <>
                      <h3 className="font-display text-xl font-semibold">🔒 Full Portfolio Access</h3>
                      <p className="mt-2 text-sm text-muted-foreground">
                        Subscribe to view {profile.name?.split(" ")[0] || "this candidate"}'s complete solutions, AI evaluation, and submission details.
                      </p>
                      <button onClick={() => setUpgradeOpen(true)} className="mt-4 rounded-xl bg-primary px-5 py-2.5 text-sm font-medium text-primary-foreground hover:opacity-90">
                        Upgrade Plan →
                      </button>
                    </>
                  ) : (
                    <>
                      <p className="text-base">Set up a recruiter account to view full portfolios</p>
                      <Link to="/recruiters" className="mt-4 inline-flex rounded-xl bg-primary px-5 py-2.5 text-sm font-medium text-primary-foreground hover:opacity-90">
                        Become a Recruiter →
                      </Link>
                    </>
                  )}
